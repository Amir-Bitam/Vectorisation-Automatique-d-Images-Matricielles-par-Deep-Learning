...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{transf_encself.png}
	\caption[Structure simplifiée d'un encodeur Transformer]{Structure simplifiée d'un encodeur Transformer, composé d'un mécanisme d'auto-attention multi-têtes et d'un réseau feed-forward. Figure adaptée de dvgodoy, sous licence CC BY 4.0.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{vit_model.png}
    \caption[Principe général d'un Vision Transformer]{Principe général d'un Vision Transformer : l'image est divisée en patchs, projetée sous forme d'embeddings, puis traitée par un encodeur Transformer. Figure adaptée de dvgodoy, sous licence CC BY 4.0.}
    \label{fig:vit_model}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption[Attention par produit scalaire mise à l'échelle]{Principe de l'attention par produit scalaire mise à l'échelle, calculée à partir des matrices $Q$, $K$ et $V$. Figure adaptée de dvgodoy, sous licence CC BY 4.0.}
    \label{fig:scaled_dot_product_attention}
\end{figure}

Ce mécanisme est illustré dans la figure~\ref{fig:scaled_dot_product_attention} et peut être formalisé par l'équation suivante \cite{vaswani2017attention} :
...
\begin{longtable}{p{1.7cm} p{2.7cm} p{2.6cm} p{3.0cm} p{3.0cm}}
\caption{Synthèse comparative des principales approches de vectorisation étudiées.}
\label{tab:synthese_vectorisation}\\

\toprule
\textbf{Méthode} & \textbf{Principe} & \textbf{Points forts} & \textbf{Limites} & \textbf{Apport} \\
\midrule
\endfirsthead
