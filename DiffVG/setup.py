    ...
    def build_extension(self, ext):
        if isinstance(ext, CMakeExtension):
            ...
            if platform.system() == "Windows":
                ...
                # Conda Python is not always found by CMake's legacy FindPythonLibs.
                cmake_args += ['-DPYTHON_INCLUDE_DIR=' + include_path,
                               '-DPYTHON_INCLUDE_DIRS=' + include_path,
                               '-DPYTHON_LIBRARY=' + python_library,
                               '-DPYTHON_LIBRARIES=' + python_library,
                               '-DPYTHON_MODULE_EXTENSION=' + sysconfig.get_config_var('EXT_SUFFIX')]
            ...
            if platform.system() == "Windows":
                ...
                is_ninja = 'Ninja' in cmake_generator
                cmake_args += ['-DCMAKE_LIBRARY_OUTPUT_DIRECTORY_{}={}'.format(cfg.upper(), extdir),
                               '-DCMAKE_RUNTIME_OUTPUT_DIRECTORY_{}={}'.format(cfg.upper(), extdir)]
                if sys.maxsize > 2**32 and not is_ninja:
                    cmake_args += ['-A', 'x64']
                build_args += ['--', '-j8' if is_ninja else '/m']
            else:
